...
En realidad este ejercicio es una versión un pelín más elaborada del siguiente. Para este ejercicio hemos creado un GameObject vacío con un componente que hemos llamado *AutoScroll* que lo que hace es, teniendo referencias a la cámara principal de la escena y a dos *SpriteRenderer* (que corresponden al fondo del juego) idénticos y colocados en escena uno junto al otro y alineados con la cámara, los desplaza pasivamente en el eje horizontal a la velocidad que le indiquemos y, cuando uno de los fondos se salga del rango de visión de la cámara, lo coloque al otro lado del otro fondo para que vuelva a estar dentro del mismo. Este es el código resultante:
